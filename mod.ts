import { Client, ClientConfig, replaceParams } from "https://deno.land/x/mysql/mod.ts";
...
export function define<M extends ModelSchema>(name: string, def: M) {
    const defaultDef = {
        createdAt: { type: FieldTypes.DATE, default: DefaultValues.CURRENT_TIMESTAMP },
        updatedAt: { type: FieldTypes.DATE, default: DefaultValues.CURRENT_TIMESTAMP, autoUpdate: true },
    };
...
export function where(where: Object): string {
    if (!where) return "";
    return "WHERE " + Object.keys(where).map(key => {
        let val = where[key];
        // If the type of field value is a function, treat it as a custom OP
        if (typeof val === "function") return replaceParams("?? ", [key]) + val();
        return replaceParams(`?? = ?`, [key, val]);
    }).join("AND ");
}
const buildWhere = where;

export class Model<M extends ModelSchema, T = ModelReturn<M>, W = ModelWhere<M>> {
    ...
    async findById(id: string | number): Promise<T> {
        assert(this.pk != null, `No field was designated as primary key in ${this.name}`);
        const pk = camel2line(this.pk.name);
        const result = await client.query(`SELECT * FROM ?? WHERE ?? = ?`, [this.name, pk, id]);
    ...
    async findAll(where: any, params?: any): Promise<T[]> {
        if (typeof where === "string") {
            const rows: any[] = await client.query(`SELECT * FROM ?? ${where}`, [this.name, ...params]);
            return rows.map(row => this.parseModel(row));
        } else {
            const rows: any[] = await client.query(`SELECT * FROM ?? ${buildWhere(where)}`, [this.name]);
            return rows.map(row => this.parseModel(row));
        }
    }
    ...
    async insert(data: T): Promise<string | number> {
        const result = await client.execute(
            `INSERT INTO ?? ?? VALUES ?`,
            [this.name, Object.keys(data), Object.values(data)]
        );
    ...
    async update(data: T, where?: T | string): Promise<number> {
        ...
        let val = Object.keys(data).map(key => {
            return replaceParams(`?? = ?`, [key, data[key]]);
        }).join(", ");
        const result = await client.execute(
            `UPDATE ?? SET ${val} ${where}`,
            [this.name]
        );
    ...
    async delete(where: any, params?: any) {
        if (typeof where === "string") {
            const result = await client.execute(`DELETE FROM ?? WHERE ${where}`, [this.name, ...params]);
            return result.affectedRows;
        } else {
            const result = await client.execute(`DELETE FROM ?? ${buildWhere(where)}`, [this.name]);
            return result.affectedRows;
        }
    }
